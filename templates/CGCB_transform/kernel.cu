...
// Funkcja log-factorial (log(n!))
double log_factorial(double n) {
    if (n < 0) return -std::numeric_limits<double>::infinity();
    if (n == 0) return 0.0;
    // Używamy funkcji lgamma, która jest log(Gamma(x)) i Gamma(n+1) = n!
    // lgamma(n+1) = log(n!)
...
double wigner_3j(double j1, double j2, double J, double m1, double m2, double M) {
    ...
    // Używamy logarytmów silni do obliczenia pre-factora
    ...
    // Sumowanie - teraz musimy dynamicznie określić zakres k, 
    // aby uniknąć silni z liczb ujemnych
    double sum_val = 0.0;
    ...
    for (int k = k_min; k <= k_max; ++k) {
        // Obliczamy mianownik w skali logarytmicznej
    ...
    // Wynik końcowy: exp(log_pre_factor) * sum_val
